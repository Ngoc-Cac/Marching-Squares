...
def edge_lerp(
    ...
    :param int or float t: The parameter at which to evaluate the interpolation.
    :param int or float v_1: The value of first corner. This should be the corner
        where there is no offset from the the top left corner.
    :param int or float v_1: The value of the second corner. This is the corner
        with the offset.

    :return float: A value representing the offset on the edge.
    :raise ZeroDivisionError: if `v_1 == v_2`
    """
...
def draw_contours(
    ...
    **This list should be read in increments of 2**.
    ...
    :param numpy.ndarray grid: A `numpy.ndarray` of shape `(rows, cols)`. Each element
        should be a scalar.
    :param int or float threshold: A number representing the threshold to seperate the
        scalars. The scalars will be divided into two groups using the threshold: one
        being greater than the threshold and one being less than or equal to the threshold.
    :param bool lerp: Whether to use linear interpolation to determine the coordinates
        of the edge. If linear interpolation is not used, the coordinates of the edge is
        at the midpoint between two corners. This defaults to False.

    :return list[tuple[float, float]]: A list of tuples of floats. For more information,
        see above.
    """
    lines = []
